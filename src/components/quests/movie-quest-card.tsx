// src/components/quests/movie-quest-card.tsx
"use client";
...
interface MovieQuestCardProps {
  task: MilestoneTask;
  monthId: string;
}

export function MovieQuestCard({ task, monthId }: MovieQuestCardProps) {
  // 解析已保存的内容
  ...
  const canSubmit =
    canEdit &&
    totalWords >= MOVIE_REVIEW_MIN_WORDS &&
    totalWords <= MOVIE_REVIEW_MAX_WORDS;
  ...
  const wordCountColor = () => {
    if (totalWords >= MOVIE_REVIEW_MIN_WORDS && totalWords <= MOVIE_REVIEW_MAX_WORDS)
      return "text-primary";
    if (totalWords > MOVIE_REVIEW_MAX_WORDS) return "text-destructive";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <p className="text-xs font-medium text-foreground/70">观影思考题：</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-mono ${wordCountColor()}`}>
            {totalWords} / {MOVIE_REVIEW_MIN_WORDS}-{MOVIE_REVIEW_MAX_WORDS} 字
          </span>
          {lastSaved && (

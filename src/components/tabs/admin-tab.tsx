...
export function AdminTab({ pendingProofs, playerData, currentWeekNum }: { pendingProofs: any[], playerData: PlayerData, currentWeekNum: number }) {
  ...
  const handleMonthlyPointSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.from('monthly_school_points').insert([{
      player_id: playerData.name,
      month_id: month,
    ...
    if (error) alert("录入失败 (可能表未建): " + error.message);
              ...
              <div key={proof.id} className="p-4 flex flex-col gap-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-1">{proof.player} • {proof.type} • {proof.date}</div>
                  </div>
                  <div className="text-lg font-mono font-semibold text-primary">&yen;{proof.reward}</div>
                </div>
                <div className="flex gap-2 mt-3">
                  <button className="flex-1 py-2 bg-primary/10 text-primary border border-primary/20 rounded-md text-xs font-medium hover:bg-primary/20 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/40">
                    予以通过
                  </button>
                  <button className="flex-1 py-2 bg-destructive/10 text-destructive border border-destructive/20 rounded-md text-xs font-medium hover:bg-destructive/20 transition-colors focus:outline-none focus:ring-2 focus:ring-destructive/40">
                    驳回重做
                  </button>
                </div>
              </div>
